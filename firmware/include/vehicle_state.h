...
struct VehicleState {
    ...
    int16_t hsi_power = 0;             // 0x247 HSI_VALUE, -100..+255 (neg=CHG, pos=PWR);
                                        // sign comes from HSI_ZONE, not from the raw byte, because
                                        // the two sides' raw ranges overlap -- see can_decoder.cpp
