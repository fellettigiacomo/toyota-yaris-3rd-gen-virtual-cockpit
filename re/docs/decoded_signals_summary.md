...
Quick reference for what has been reverse-engineered on this vehicle's CAN
bus. Full methodology and validation evidence: [`signal_findings.md`](signal_findings.md).
Machine-readable definitions: [`../../dbc/toyota_yaris_xp130_reversed.dbc`](../../dbc/toyota_yaris_xp130_reversed.dbc).
...
- Every gauge-relevant signal on the dash is decoded: speed, RPM, gear, turn
  signals, accelerator, brake, ambient temperature, and the CHG/ECO/PWR
  power-flow needle.
- The HV battery **SOC** and an explicit **EV-drive bit** are plain passive
  broadcasts — no active diagnostic polling needed.
...
- HV battery current/voltage and remaining driving range are confirmed
  absent from this bus; not obtainable by passive listening.
